...
# Detect GitHub organization
    read -rp "Please enter your GitHub organization name: " ORG
    if [[ -z "$ORG" ]]; then
...
if [ -n "$LOCAL_PATH" ] && [ -d "$LOCAL_PATH" ]; then
    ...
    while IFS= read -r -d '' REPO_GIT; do
        ...
        if grep -r "tj-actions/changed-files" "$REPO_DIR/.github" 2>/dev/null; then
            ...
            find "$REPO_DIR/.github" -type f \( -name "*.yml" -o -name "*.yaml" \) -print0 | xargs -0 grep -l "tj-actions/changed-files" 2>/dev/null >> "$OUTPUT_DIR/${REPO_NAME}_affected_files.txt"
        else
            echo "$REPO_NAME: CLEAR"
        fi
    done < <(find "$LOCAL_PATH" -name ".git" -type d -prune -print0)
